/*
  ...
  gengetopt -i edo.ggo --file-name=edo_cmdl --include-getopt --default-optional --func-name=edo_cmdline_parser --arg-struct-name=edo_args_info
...
const char *edo_args_info_purpose = "A tool for selecting ensemble-diversity-optimized barcodes for cDNA libraries";

const char *edo_args_info_usage = "Usage: " EDO_CMDLINE_PARSER_PACKAGE " [OPTIONS]...";

const char *edo_args_info_versiontext = "";

const char *edo_args_info_description = "";

const char *edo_args_info_help[] = {
...
typedef enum {ARG_NO
  , ARG_FLAG
  , ARG_STRING
} edo_cmdline_parser_arg_type;

static
void clear_given (struct edo_args_info *args_info);
static
void clear_args (struct edo_args_info *args_info);

static int
edo_cmdline_parser_internal (int argc, char **argv, struct edo_args_info *args_info,
                        struct edo_cmdline_parser_params *params, const char *additional_error);
...
static
void init_args_info(struct edo_args_info *args_info)
{
...
void
edo_cmdline_parser_print_version (void)
{
  printf ("%s %s\n",
     (strlen(EDO_CMDLINE_PARSER_PACKAGE_NAME) ? EDO_CMDLINE_PARSER_PACKAGE_NAME : EDO_CMDLINE_PARSER_PACKAGE),
     EDO_CMDLINE_PARSER_VERSION);

  if (strlen(edo_args_info_versiontext) > 0)
    printf("\n%s\n", edo_args_info_versiontext);
}

static void print_help_common(void) {
  edo_cmdline_parser_print_version ();

  if (strlen(edo_args_info_purpose) > 0)
    printf("\n%s\n", edo_args_info_purpose);

  if (strlen(edo_args_info_usage) > 0)
    printf("\n%s\n", edo_args_info_usage);

  printf("\n");

  if (strlen(edo_args_info_description) > 0)
    printf("%s\n\n", edo_args_info_description);
}

void
edo_cmdline_parser_print_help (void)
{
  int i = 0;
  print_help_common();
  while (edo_args_info_help[i])
    printf("%s\n", edo_args_info_help[i++]);
}

void
edo_cmdline_parser_init (struct edo_args_info *args_info)
{
...
void
edo_cmdline_parser_params_init(struct edo_cmdline_parser_params *params)
{
  if (params)
    { 
      params->override = 0;
...
edo_cmdline_parser_params_create(void)
{
  struct edo_cmdline_parser_params *params = 
    (struct edo_cmdline_parser_params *)malloc(sizeof(struct edo_cmdline_parser_params));
  edo_cmdline_parser_params_init(params);  
  return params;
}
...
static void
edo_cmdline_parser_release (struct edo_args_info *args_info)
{
...
int
edo_cmdline_parser_dump(FILE *outfile, struct edo_args_info *args_info)
{
  int i = 0;

  if (!outfile)
    {
      fprintf (stderr, "%s: cannot dump options to stream\n", EDO_CMDLINE_PARSER_PACKAGE);
      return EXIT_FAILURE;
    }
...
int
edo_cmdline_parser_file_save(const char *filename, struct edo_args_info *args_info)
{
  FILE *outfile;
  int i = 0;
  ...
  if (!outfile)
    {
      fprintf (stderr, "%s: cannot open file for writing: %s\n", EDO_CMDLINE_PARSER_PACKAGE, filename);
      return EXIT_FAILURE;
    }

  i = edo_cmdline_parser_dump(outfile, args_info);
  fclose (outfile);
...
void
edo_cmdline_parser_free (struct edo_args_info *args_info)
{
  edo_cmdline_parser_release (args_info);
}
...
int
edo_cmdline_parser (int argc, char **argv, struct edo_args_info *args_info)
{
  return edo_cmdline_parser2 (argc, argv, args_info, 0, 1, 1);
}

int
edo_cmdline_parser_ext (int argc, char **argv, struct edo_args_info *args_info,
                   struct edo_cmdline_parser_params *params)
{
  int result;
  result = edo_cmdline_parser_internal (argc, argv, args_info, params, 0);
...
int
edo_cmdline_parser2 (int argc, char **argv, struct edo_args_info *args_info, int override, int initialize, int check_required)
{
  int result;
  struct edo_cmdline_parser_params params;
...
int
edo_cmdline_parser_required (struct edo_args_info *args_info, const char *prog_name)
{
  FIX_UNUSED (args_info);
 ...
 * @param check_ambiguity @see edo_cmdline_parser_params.check_ambiguity
 * @param override @see edo_cmdline_parser_params.override
...
int update_arg(void *field, char **orig_field,
               ...
               edo_cmdline_parser_arg_type arg_type,
...
int
edo_cmdline_parser_internal (
  int argc, char **argv, struct edo_args_info *args_info,
                        struct edo_cmdline_parser_params *params, const char *additional_error)
{
  int c;	/* Character of the parsed option.  */

  int error_occurred = 0;
  struct edo_args_info local_args_info;
  ...
  if (initialize)
    edo_cmdline_parser_init (args_info);

  edo_cmdline_parser_init (&local_args_info);
...
.  */
          ...
          if (update_arg( (void *)&(args_info->lab_settings_arg), 
               &(args_info->lab_settings_orig), &(args_info->lab_settings_given),
...
.  */
          if (strcmp (long_options[option_index].name, "eval") == 0)
          {
            ...
            if (update_arg((void *)&(args_info->eval_flag), 0, &(args_info->eval_given),
                &(local_args_info.eval_given), optarg, 0, 0, ARG_FLAG,
                check_ambiguity, override, 1, 0, "eval", '-',
                additional_error))
              goto failure;
        ...
        default:	/* bug: option not considered.  */
          fprintf (stderr, "%s: option unknown: %c%s\n", EDO_CMDLINE_PARSER_PACKAGE, c, (additional_error ? additional_error : ""));
          abort ();
        } /* switch */
    } /* while */
  ...
  edo_cmdline_parser_release (&local_args_info);
